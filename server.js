...
const pcaptotext = (files) => {
  return new Promise((resolve, reject) => {
    exec(`echo aalagung06 | sudo -S tcpdump -A -r ${files} > csv/output.txt`, (err, stdout, stderr) => {
    if (err) {
        console.warn(`exec error: ${err}`);
      }
      resolve(stdout? stdout:stderr)
...
const startInterFace = (interface) => {
  return new Promise((resolve, reject) => {
    exec(`echo aalagung06 | sudo -S airmon-ng start ${interface}`, (err, stdout, stderr) => {
    if (err) {
        console.warn(`exec error: ${err}`);
      }
      resolve(stdout? stdout:stderr)
...
const monitoroff = (interface) => {
  return new Promise((resolve, reject) => {
    exec(`echo aalagung06 |sudo airmon-ng stop ${interface}`, (stdout,stderr,error)=>{
      if(error) {
